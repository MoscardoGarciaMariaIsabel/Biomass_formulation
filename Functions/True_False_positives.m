...
for z = 1:size(CellLine,1)
    if any((ismember(binaryDepScores.Properties.VariableNames,CellLine(z)))==1)
        ...
        N = numel(GeneList{z}); %predicted EG
        Non_EG_names = setdiff(GeneList_all,GeneList{z}); %Predicted non essential genes
        ...
        x = [x;sum(ismember(lower(GeneList{z}), all_genes))]; %number of predicted EG that are known EG
